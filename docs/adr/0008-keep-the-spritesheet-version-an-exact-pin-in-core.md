...
Data carries sprite coordinates; core carries the URL of the sheet image those
...
- The failure mode is silent. A stale pin raises no error; it draws every
  non-native emoji from the wrong cell of the sheet. Nothing but a check catches
  it, so the gate renders a non-native Set in a browser and asserts each sprite's
...
- Consumers who pass `getSpritesheetURL` own their own copy of this pin. A core
  minor that moves the dataset requires them to move their sheet with it; the
  CHANGELOG has to say so every time.
